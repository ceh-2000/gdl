...
def log_loaded_dataset(dataset, format, name):
    ...
    logging.info(f"  undirected: {dataset[0].is_undirected()}") #MOD uncomment
...
def load_dataset_master(format, name, dataset_dir):
    ...
    pre_transform_in_memory(dataset, partial(task_specific_preprocessing, cfg=cfg)) #MOD
    ...
    print("--------------------Begining splitting")
    # Verify or generate dataset train/val/test splits
    prepare_splits(dataset)
    print("--------------------Finish splitting")

    # Precompute in-degree histogram if needed for PNAConv.
    if cfg.gt.layer_type.startswith('PNA') and len(cfg.gt.pna_degrees) == 0: #MOD
...
def preformat_GNNBenchmarkDataset(dataset_dir, name):
    ...
    elif name in ['PATTERN', 'CLUSTER', 'CSL']:  #MOD
        tf_list = []
    else:
...
def preformat_OGB_PCQM4Mv2(dataset_dir, name):
    ...
    if name == 'full':
        ...
        data_list = [data for data in dataset] #MOD
        dataset._indices = None #MOD
        dataset._data_list = data_list #MOD
        dataset.data, dataset.slices = dataset.collate(data_list) #MOD
