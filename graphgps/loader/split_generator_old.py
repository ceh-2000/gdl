...
def prepare_splits(dataset):
    ...
    elif split_mode == "fixed": #MOD
        setup_fixed_split(dataset) #MOD
    elif split_mode == "sliced": #MOD
        setup_sliced_split(dataset) #MOD
    else:
...
def setup_random_split(dataset):
    ...
    #if isinstance(split_ratios[0], float): #MOD
    #    val_test_ratio = split_ratios[1] / (1 - split_ratios[0]) #MOD
    #else:  #MOD
    #    val_test_ratio = split_ratios[1] #MOD
...
def set_dataset_splits(dataset, splits):
    ...
    if task_level == 'node':
        ...
        #the following is relevant when running with NeuralGraph, otherwise index is  an array
        #if splits[0].dtype == np.int32: #MOD
        #    splits = [s.astype(np.int64) for s in splits] #MOD
        for split_name, split_index in zip(split_names, splits):
